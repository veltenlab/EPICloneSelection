validateMBDesigner <- function(csv,
                            config='/home/mscherer/cluster/project/Methylome/src/selection_pipeline/config.yaml'){
  #.libPaths(c('/home/mscherer/R/',.libPaths()))
  ...
  while(i < num){
    ...
    for(j in 1:length(tfs.all)){
      ...
      if(length(op)>0){
        dat[i,paste0(tfs.all[j], '_ChIP')] <- paste0(tfs.all[j], '_ChIP')
      }else{
        dat[i,paste0(tfs.all[j], '_ChIP')] <- NA
      }
    }
...
res <- validateMBDesigner('/home/mscherer/cluster/project/Methylome/infos/HSCs/CO475.design-summary.csv')
write.csv(res, '/home/mscherer/cluster/project/Methylome/infos/HSCs/C0475-design-summary_annotated.csv')
# mb_id <- '4318'
# res <- validateMBDesigner(paste0('/home/mscherer/cluster/project/Methylome/analysis/selection_pipeline/MB_output/Designer/', mb_id, '-design-summary.csv'))
 input <- read.csv('/home/mscherer/cluster/project/Methylome/analysis/selection_pipeline/MB_input/CpGs_only.csv')
...
# annotate TFBS with narrower window around CpG
 require(yaml)
 require(data.table)
 require(RnBeads)
 require(RnBeads.mm10)
 require(TFBSTools)
 suppressPackageStartupMessages(library(BSgenome.Mmusculus.UCSC.mm10))
res <- read.csv('/home/mscherer/cluster/project/Methylome/infos/HSCs/C0475-design-summary_annotated.csv') 
 config <- yaml.load_file('/home/mscherer/cluster/project/Methylome/src/selection_pipeline/config.yaml')
 res$pu1_motif <- NA
 res$pu1_chip <- NA
 pu1.chip <- read.table(config[['annotations']][['pu1_chip']])
 pu1.chip$V2 <- as.numeric(pu1.chip$V2)
 ...
 genome <- BSgenome.Mmusculus.UCSC.mm10
 hema.motifs <- read.csv(config[['annotations']][['hema_tf_motifs']])
 hema.motifs.pwm <- readRDS(config[['annotations']][['hema_tf_motifs_pwm']])
 hema.motifs.pwm <- hema.motifs.pwm[sapply(names(hema.motifs.pwm), function(x)any(unlist(sapply(hema.motifs$TF, function(y)(grepl(y, x))))))]
 names(hema.motifs.pwm) <- sapply(strsplit(names(hema.motifs.pwm), '-'), function(x)x[2])
 hema.motifs.pwm <- hema.motifs.pwm[!grepl("[:]", names(hema.motifs.pwm))]
 pu1.motif <- DNAString(config[['general']][['pu1_motif']])
 res[hema.motifs$TF] <- NA
 tfs.files.all <- list.files(config[['annotations']][['tf_chip']],full.names=TRUE)
 tfs.all <- gsub('.bed', '', list.files(config[['annotations']][['tf_chip']]))
 for(tf in tfs.all){
   res[,paste0(tf, '_ChIP')] <- NA
 }
for(i in 1:nrow(res)){
  print(i)
  dat <- res[i, ]
  chr <- as.character(res[i,"chr"])
  seq <- genome[[chr]]
  region.seq <- seq[(as.numeric(as.character(dat['Start_CpG']))-100):(as.numeric(as.character(dat['End_CpG']))+100)]
  region.gr <- GRanges(paste0(chr, ':',
                              (as.numeric(as.character(dat['Start_CpG']))-100),
                              '-',
                              (as.numeric(as.character(dat['End_CpG']))+100)))
  pattern.match <- matchPattern(pu1.motif, region.seq, max.mismatch = 1)
  if(length(pattern.match)>0){
    res[i, 'pu1_motif'] <- 'PU1'
  }
  ...
  for(j in 1:length(tfs.all)){
    ...
    op <- findOverlaps(region.gr,tf.gr)
    if(length(op)>0){
      res[i,paste0(tfs.all[j], '_ChIP')] <- paste0(tfs.all[j], '_ChIP')
    }else{
      res[i,paste0(tfs.all[j], '_ChIP')] <- NA
    }
  }
} 
 row.names(res) <- res$AmpID
...
# ggsave('/home/mscherer/cluster/project/Methylome/analysis/selection_pipeline/plots/amplicons_aging.pdf',
#        height=100,
#        width=100,
#        unit='mm')
# 
...
# }
# plot <- ggplot(to_plot, aes(x="", y=freq, fill=x))+geom_bar(stat="identity", width=1, color="white")+plot_theme+scale_fill_tron()+ylab("")+xlab("")+facet_wrap(Type~., ncol=3)
# ggsave('/home/mscherer/cluster/project/Methylome/analysis/selection_pipeline/plots/amplicons_enhancer.pdf',
#        height=100,
#        width=100,
#        unit='mm')
# 
...
# ggsave('/home/mscherer/cluster/project/Methylome/analysis/selection_pipeline/plots/amplicon_type_distribution.png',
#        height=150,
#        width=150,
#        unit='mm')
# 
...
#                     panel.spacing = unit(.1, "lines"))
# 
# dat <- read.csv('/home/mscherer/cluster/project/Methylome/analysis/selection_pipeline/MB_output/Designer/4135-design-summary_annotated.csv')
# types <- gsub('[0-9]', '', dat$Type)
# types[grep('high', types)] <- 'DMC'
# types[types%in%'IMR'] <- 'IMC'
# dat$Type <- types
...
#   plot_theme
# ggsave('/home/mscherer/cluster/project/Methylome/analysis/selection_pipeline/plots/amplicon_TF_all_distribution.png',
#        height=75,
#        width=150,
#        unit='mm')
# 
...
# ggsave('/home/mscherer/cluster/project/Methylome/analysis/selection_pipeline/plots/amplicon_TF_stratified.png',
#        height=150,
#        width=300,
#        unit='mm')
