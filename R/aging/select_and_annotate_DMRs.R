...
report <- '/users/mscherer/cluster/project/Methylome/analysis/selection_pipeline/RnBeads/aging/rnb_report_20211109_mm10/'
...
checkForCutSite <- function(input, config, number){
  ...
  dat <- read.csv(input)
  dat <- dat[order(dat$mean.diff, decreasing=TRUE),]
  i <- 1
  num <- 0
  vec.all <- c()
  dat$regionStart <- NA
  ...
  dat$Hox <- NA
  dat$GCContent <- NA
  tfs.files.all <- list.files('/users/mscherer/cluster/project/Methylome/infos/BCells/TFBS_Wilson/bed/mm10/',full.names=TRUE)
  tfs.all <- gsub('.bed', '', list.files('/users/mscherer/cluster/project/Methylome/infos/BCells/TFBS_Wilson/bed/mm10/'))
  for(tf in tfs.all){
    dat[,tf] <- NA
  }
  genes <- unlist(rnb.get.annotation('genes',assembly = 'mm10'))
  promoters <- unlist(rnb.get.annotation('promoters',assembly = 'mm10'))
  reg.elements <- read.csv('/users/mscherer/cluster/project/Methylome/infos/BCells/enhancer_catalog_lawrence/H3K27Ac.csv')
  row.names(reg.elements) <- paste0(reg.elements$Chr,'_',reg.elements$Start)
  reg.elements.tab <- read.table('/users/mscherer/cluster/project/Methylome/infos/BCells/enhancer_catalog_lawrence/H3K27Ac_mm10.bed',sep='\t')
...
res <- checkForCutSite(file.path(report, 'differential_methylation_data/diffMethTable_site_cmp1.csv'),
                       config = config,
                       number = 100)